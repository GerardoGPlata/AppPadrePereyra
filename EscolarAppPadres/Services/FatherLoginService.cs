...
{
    public class FatherLoginService
    {
        private readonly HttpClient _httpClient;

        public FatherLoginService()
        {
        ...
        public async Task<ResponseModel<TokenResponse>?> LoginFatherAsync(LoginFather loginFather)
        {
            var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));

            try
            {
                var json = JsonSerializer.Serialize(loginFather);
                var content = new StringContent(json, Encoding.UTF8, "application/json");

                var response = await _httpClient.PostAsync(ApiRoutes.FatherLogin.LoginFather, content, cts.Token);
        ...
        public async Task<ResponseModel<object>?> ChangePasswordAsync(ChangePassword changePasswordRequest, string token)
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync(ApiRoutes.FatherLogin.changePassword, content, cts.Token);
                var responseContent = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ResponseModel<Token>?> RefreshTokenAsync(Token token)
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync(ApiRoutes.FatherLogin.RefreshToken, content, cts.Token);
                var responseContent = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ResponseModel<object>?> LogoutAsync(Logout logoutRequest, string token)
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync(ApiRoutes.FatherLogin.Logout, content, cts.Token);
                var responseContent = await response.Content.ReadAsStringAsync();
